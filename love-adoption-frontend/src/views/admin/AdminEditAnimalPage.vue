<template>
  <div class="admin-edit-animal-page">
    ...
    <div v-else-if="animalForm" class="form-container">
      <el-form
        :model="animalForm"
        :rules="rules"
        ref="animalFormRef"
    ...
    <div v-else class="error-container">
      ...
      <el-button type="primary" @click="$router.push('/admin/animals')">返回列表</el-button>
    </div>
  </div>
</template>
...
import { getAnimalByIdApi, updateAnimalApi } from '../../api/animal'
...
const handleImageSuccess = (response) => {
  animalForm.value.image_url = response
...
const submitForm = () => {
  animalFormRef.value.validate(async (valid) => {
    ...
    try {
      await updateAnimalApi(animalForm.value)
      ElMessage.success('更新动物信息成功')
      router.push('/admin/animals')
    } catch (error) {
      console.error('更新动物信息失败:', error)
      ElMessage.error('更新动物信息失败')
    } finally {
...
.loading-container, .form-container, .error-container {
